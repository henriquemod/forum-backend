...
interface SutTypes {
  ...
  postManager: Post.UpdatePost & Post.FindPost
...
const makeSut = (): SutTypes => {
  ...
  return {
    sut: new UpdatePostController(postManager, userManager),
    postManager,
    userManager
  }
}
...
describe('Update Post Controller', () => {
  ...
  it('should return statusCode 403 if user do not own the post', async () => {
    ...
    expect(res.error).toBe('You are not allowed to edit this post')
  })
  ...
  it('should return statusCode 500 if createPost throws', async () => {
    ...
    jest.spyOn(postManager, 'updatePost').mockRejectedValueOnce(new Error())
